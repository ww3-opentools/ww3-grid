			   ...
			   /*short fill_value)*/
{
  ...
  g_assert(status == NC_NOERR);
  status = nc_inq_varid (ncid, depthvar, &zid);
  g_assert(status == NC_NOERR);

  int londimid, latdimid;
  status = nc_inq_dimid (ncid, lonvar, &londimid);
  g_assert(status == NC_NOERR);
  status = nc_inq_dimid (ncid, latvar, &latdimid);
  g_assert(status == NC_NOERR);
  ...
  if (status != NC_NOERR) {
      fprintf(stdout, "---- WARNING: No _FillValue for %s \n", depthvar);
  }
			     ...
			     PyObject * _dlat)
{
  ...
  char * depthvar = NULL, * lonvar = NULL, * latvar = NULL;
  ...
  if ( _depthvar == Py_None) {
    PyErr_SetString(PyExc_TypeError, "Error depthvar is NULL.\n");
    return NULL;
  }
  else {
    if ( !PyUnicode_Check(_depthvar) ) {
      PyErr_SetString(PyExc_TypeError, "Error depthvar type error, string expected.\n");
      return NULL;
    }
    if ( PyUnicode_KIND(_depthvar) != PyUnicode_1BYTE_KIND ) {
      PyErr_SetString(PyExc_TypeError, "Error depthvar type error, PyUnicode_1BYTE_DATA expected.\n");
      return NULL;
    }
    depthvar = PyUnicode_1BYTE_DATA(_depthvar);
  }
  if ( _lonvar == Py_None) {
    PyErr_SetString(PyExc_TypeError, "Error lonvar is NULL.\n");
    return NULL;
  }
  else {
    if ( !PyUnicode_Check(_lonvar) ) {
      PyErr_SetString(PyExc_TypeError, "Error lonvar type error, string expected.\n");
      return NULL;
    }
    if ( PyUnicode_KIND(_lonvar) != PyUnicode_1BYTE_KIND ) {
      PyErr_SetString(PyExc_TypeError, "Error lonvar type error, PyUnicode_1BYTE_DATA expected.\n");
      return NULL;
    }
    lonvar = PyUnicode_1BYTE_DATA(_lonvar);
  }
  if ( _latvar == Py_None) {
    PyErr_SetString(PyExc_TypeError, "Error latvar is NULL.\n");
    return NULL;
  }
  else {
    if ( !PyUnicode_Check(_latvar) ) {
      PyErr_SetString(PyExc_TypeError, "Error latvar type error, string expected.\n");
      return NULL;
    }
    if ( PyUnicode_KIND(_latvar) != PyUnicode_1BYTE_KIND ) {
      PyErr_SetString(PyExc_TypeError, "Error latvar type error, PyUnicode_1BYTE_DATA expected.\n");
      return NULL;
    }
    latvar = PyUnicode_1BYTE_DATA(_latvar);
  }
				   ...
				   PyObject *keywds)
{
  ...
  static char *kwlist[] = {"file_name_in", "file_name_out", "depthvar",
                           "lonvar", "latvar", "lon0", "lon1", "dlon", 
                            "lat0", "lat1", "dlat", NULL};

  if (!PyArg_ParseTupleAndKeywords(args, keywds, "OOOOOOOOOOO", kwlist,
                                   &_file_name_in, &_file_name_out,
                                   &_depthvar, &_lonvar, &_latvar,
                                   &_lon0, &_lon1, &_dlon,
				                   &_lat0, &_lat1, &_dlat)) {
...
int main ()
{
  /*return _bathy_interpolate("../../example_data/etopo1.nc",*/
				/*"/tmp/glob.nc",*/
				/*"z","lat","lon",*/
				/*0, 360, 4.0,*/
				/*-70, 70, 4.0);*/
				/*[>-32767);<]*/
  return _bathy_interpolate("../../example_data/GEBCO_2014_2D.nc",
                "/tmp/NZ.nc",
                "elevation","lon","lat",
                155, 180, 1.0,
                -70, -30, 1.0);
