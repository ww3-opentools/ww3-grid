...
!  Read Global and Arctic part Multiple-Cell info
       ...
       OPEN(UNIT=8, FILE= TRIM(FNAME),STATUS='OLD',IOSTAT=nn,ACTION='READ')
       IF(nn /= 0) PRINT*,  TRIM(FNAME)//' was not opened! '
         READ (8,*) NGLo, NRLCel(1:MRL) 
       DO J=1,NGLo
         READ (8,*) ICE(1,J), ICE(2,J), ICE(3,J), ICE(4,J), KG(J)
       END DO
       CLOSE(8)
       PRINT*,  TRIM(FNAME)//' read done ', NGLo, NRLCel(1:MRL) 

!!  Arctic part becomes optional.  JGLi12Dec2011
       IF( ARCTIC ) THEN
         PRINT*, ' Opening '//TRIM(AFNAME)//' ...' 
         OPEN(UNIT=9, FILE= TRIM(AFNAME),STATUS='OLD',IOSTAT=nn,ACTION='READ')
         IF(nn /= 0) PRINT*,  TRIM(AFNAME)//' was not opened! '
            READ (9,*) NArc, NArB, NGLB
         DO J=NGLo+1, NGLo+NArc
            READ (9,*) ICE(1,J), ICE(2,J), ICE(3,J), ICE(4,J), KG(J)
         END DO
         CLOSE(9)
         PRINT*,  TRIM(AFNAME)//' read done  NArc=', NArc
...
!!   IF(ARCTIC) except for the polar (last) cell, as set above.
       DO L=-9, LMN
         J=ICE(2,L)
...
!!  Read sorted ISD JSD variables for global part.
       PRINT*, ' Opening '//TRIM(INAME)//' ...'
       OPEN(UNIT=10, FILE= TRIM(INAME),STATUS='OLD',IOSTAT=nn,ACTION='READ')
       IF(nn /= 0) PRINT*,  TRIM(INAME)//' was not opened!'
       ...
       PRINT*, ' Opening '//TRIM(JNAME)//' ...'
       OPEN(UNIT=11, FILE= TRIM(JNAME),STATUS='OLD',IOSTAT=nn,ACTION='READ')
       IF(nn /= 0) PRINT*,  TRIM(JNAME)//' was not opened!'
...
!!  Read sorted ISD JSD variables for Arctic part.
       IF(ARCTIC) THEN
         ...
         PRINT*, ' Opening '//TRIM(AINAME)//' ...'
         OPEN(UNIT=10, FILE= TRIM(AINAME),STATUS='OLD',IOSTAT=nn,ACTION='READ')
         IF(nn /= 0) PRINT*,  TRIM(AINAME)//' was not opened!'
         READ(10,*) NUAr
         ...
         PRINT*, ' Opening '//TRIM(AJNAME)//' ...'
         OPEN(UNIT=11, FILE= TRIM(AJNAME),STATUS='OLD',IOSTAT=nn,ACTION='READ')
         IF(nn /= 0) PRINT*,  TRIM(AJNAME)//' was not opened!'
         READ(11,*) NVAr
...
!!------------------------------------------------------------------------
!! SUBROUTINE GMCxUNO2
!! Subroutine that calculates mid-flux values for x dimension 
...
!!------------------------------------------------------------------------
!! SUBROUTINE Specturn
!!  This subroutine turns the wave spectrum by an fixed angle anti-clockwise
...
!!------------------------------------------------------------------------
!! SUBROUTINE LLTOEQANGLE
!! 
!!   Merged UM source code for rotated grid, consiting the following
!!   original subroutines in UM 6.1
!!     LLTOEQ1A  WCOEFF1A  and  LBCROTWINDS1
!!   The last subroutine is modified to process only one level winds
!!   cpp directives are removed and required header C_Pi.h inserted.
!! 	    Jian-Guo Li     26 May 2005
!! 
!!   The WCOEFF1A subroutine is merged into LLTOEQ to reduce repetition
!!   of the same calculations. Subroutine interface changed to 
!!   LLTOEQANGLE
!! 	    Jian-GUo Li     23 Aug 2005
!! 
...
!! 	          Also calculate rotation angle in degree to tranform
!!             standard wind velocity into equatorial wind.
!! 	          Valid for 0<PHI_POLE<90 or new pole in N. hemisphere.
!!                                                                
!* Arguments:--------------------------------------------------------    
